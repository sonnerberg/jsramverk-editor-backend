...
import { documentsSchema } from '../src/db/documentsSchema.js'
dotenv.config()
...
const database = {
    recreateDB: async () => {
        ...
        try {
            ...
            db.collection(DB_DOCUMENTS_COLLECTION).drop((err, result) => {
                if (err) throw err
                if (result) console.log(`Collection successfully dropped`)
            })
            await db.createCollection(DB_DOCUMENTS_COLLECTION, documentsSchema)
        } finally {
